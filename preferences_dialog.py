...
class PreferencesDialog(QDialog):
    ...
    def _editor_visibility_rules(self, tool_name: str, mode_name: str) -> dict:
    ...
    def _build_editor_tab(self):
        intro_label = QLabel(
            ...
            "Each tool has fixed Brush Defaults and Smart Selection Defaults sections."
        )
        intro_label.setWordWrap(True)
        ...
        for tool_name in self._tool_names():
            ...
            for mode_name in self._mode_names():
                ...
                mode_label = QLabel(f"{mode_name} Defaults")
                ...
                form.setContentsMargins(12, 0, 12, 12)
                form.setHorizontalSpacing(10)
                form.setVerticalSpacing(8)

                brush_size_row, brush_size_slider, brush_size_value = self._create_slider_row(1, 200, form_widget)
                softness_row, softness_slider, softness_value = self._create_slider_row(0, 100, form_widget)
                opacity_row, opacity_slider, opacity_value = self._create_slider_row(1, 100, form_widget)
                spacing_row, spacing_slider, spacing_value = self._create_slider_row(1, 100, form_widget)
                tolerance_row, tolerance_slider, tolerance_value = self._create_slider_row(0, 255, form_widget)

                magic_mode_combo = QComboBox(form_widget)
                magic_mode_combo.addItems(["Connected Region", "Global Match"])

                edge_protect_check = QCheckBox("Enable Edge Protect", form_widget)

                rules = self._editor_visibility_rules(tool_name, mode_name)
                ...
                if rules["edge_protect"]:
                    form.addRow("", edge_protect_check)

                if not any(rules.values()):
                    empty_label = QLabel("No additional defaults for this mode.")
                    empty_label.setWordWrap(True)
                    empty_label.setStyleSheet("color: #cfcfcf; font-size: 11px;")
                    form.addRow("", empty_label)
                ...
                mode_layout.addWidget(form_widget)
                ...
                tool_layout.addWidget(mode_widget)

            self.editor_layout.addWidget(tool_group)
            self._register_search_target(
                self.editor_tab,
                tool_group,
                f"{tool_name} brush defaults smart selection defaults save preset brush size softness opacity spacing tolerance magic mode edge protect",
            )
        ...
        try:
            ...
            if imported:
                ...
                QMessageBox.information(
                    self,
                    "Preset Imported",
                    f'Imported preset "{str(data.get("preset_name", "")).strip()}".',
                )
